...
# ================= CONFIGURATION =================
BOOTSTRAP_SERVERS = ['10.214.203.73:9092']  # Broker Node (Kafka)
ADMIN_API_URL = 'http://10.214.203.13:5000'  # Admin Node (Flask)
POLL_INTERVAL = 5
...
class AdminAPIClient:
    ...
    def get_active_topics(self) -> List[str]:
        """Fetch active topics from Flask API."""
        try:
            res = requests.get(f"{self.admin_url}/api/topics/active", timeout=self.timeout)
            if res.status_code == 200:
                return [t['topic_name'] for t in res.json()]
...
# ===================================================
#  Producer System with Auto-Topic Creation
# ===================================================
class ProducerSystem:
    ...
    def data_generator_thread(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                active_topics = self.admin_api.get_active_topics()
                if not active_topics:
                    logger.info("No active topics found, waiting...")
                    time.sleep(5)
                    continue

                topic_name = random.choice(active_topics)

                # Infer data type from topic name
    ...
    def topic_watcher_thread(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                active = self.admin_api.get_active_topics()
                for topic in active:
